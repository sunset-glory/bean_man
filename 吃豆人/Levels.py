...
class Level1():
#创建角色
	def setupPlayers(self, hero_image_path, ghost_images_path, base_speed):
		self.hero_sprites = pygame.sprite.Group()
		self.ghost_sprites = pygame.sprite.Group()
		self.hero_sprites.add(Player(287, 439, hero_image_path, base_speed))
		for each in ghost_images_path:
			name = each.split('/')[-1].split('.')[0]             #通过分解路径名来得到图片上设置的名字
			if name == 'Blinky':
				ghost1 = Player(287, 199, each, base_speed)                       #设定每个敌人的初始位置
				ghost1.move = True                                 #因为敌人是电脑控制，则移动标志始终有效，以保持其运动
				...
				ghost2 = Player(319, 259, each, base_speed)
				ghost2.move = True
				...
				ghost3 = Player(255, 259, each, base_speed)
				ghost3.move = True
				...
				ghost4 = Player(287, 259, each, base_speed)
				ghost4.move = True
...
#创建墙
	def setupWalls(self, wall_color):
		...
		wall_positions = [[0, 0, 6, 600],[0, 0, 600, 6],[0, 600, 606, 6],[600, 0, 6, 606],[300, 0, 6, 66],[60, 60, 186, 6],
						  ...
						  [120, 540, 126, 6],[360, 540, 126, 6]]          #该关卡设计以及敌人行动列表参考网上关卡1
